...
{			
	...
	{
		var departHTML = '';	
		for(i=0;i<collDepartmentMaster.length;i++)
		{
			...
			for(j=0;j<CriticalCatArray.length;j++)
			{
			
			
				var	CriticalCategoryTempHtml = tempCriticalCategoryHTML;
					CriticalCategoryTempHtml=CriticalCategoryTempHtml.replace(/__CriticalCategoryName__/g,CriticalCatArray[j])
				...
				
					{
							...
							tempCriteria +='<li class="list-group-item question"><label>'+CriteriaArray[k]+'</label>'+
							    '<div class="mid" align="right">'+
								...
								'</div>'+
							'<div class="conform-section" id="showFormId__PlantDepartmentID___" style="display:none;">'+
...
{	
	...
	{
		CriteriaArray.push(collCriteriaa[i].Criteria);
...
function GetCriteriaSuccess(collEmployeeMaster)
{
	...
	if(collEmployeeMaster.length>0)
	{
		...
		{	
			no=i;
			no++;
			CriticalCatArray.push(collEmployeeMaster[i].CategoryTitle);
			
			//var newCategory = tempCriticalCriteria;
			
			//newCategory=newCategory.replace('__no.__',no);
			
			//newCategory=newCategory.replace('__CriticalCategory__',collEmployeeMaster[i].CategoryTitle);
			
			
			
			//tempCriticalHTML+=newCategory;
...
function UpdateNonConfirmity(itemId){
	...
	var Observation =$('#observationIdd').val();
	var CorrectiveAction =$('#correctiveActionId').val();
	var severity = $('#CriteriaSeverity').val();
