...
function ObservationAttachmentClickFunction(event) {
	    ...
	    fileID=event.id;
	   	...
	   	var CreateAttachmentId=fileID.replace("AttachID","ObservationClearId");

	   var AreaId=ObservId.split("_")[3];
	   var CategoryId=ObservId.split("_")[5];
	   var CriteriaId=ObservId.split("_")[7];
	   		   for(i=0;i<ObservationData.length;i++)
			{
					if(AreaId==ObservationData[i].AreaId && CategoryId== ObservationData[i].CategoryId && CriteriaId==ObservationData[i].CriteriaId)
					{
...
function fonload2() {
    ...
    if(Newfilename!='')
    {
    	createitem.executeAsync({
	        url: WebAbsoluteUrl+"/_api/web/lists/GetByTitle('"+PlantTourAttachmentsLibraryName+"')/items("+ItemID+")/AttachmentFiles/add(FileName='" + Newfilename + "')",
	        method: "POST",
	        headers: {
...
function GetObservationAttachmentsSuccess(data)
{
	...
	var ItemId=data[0].Id;
		...
		{	
		//$('#'+AttachID).append('<br/>'+parseInt(i+1)+'. <a href="'+data[i].Fileurl+'" target="_blank" style="text-decoration:underline;font-weight: bold;">'+data[i].Filename+'</a><a onclick="DeleteObservationFormAttachment(this,'+ItemId+')" data-ItemId="'+ItemId+'" data-filename="'+data[i].Filename+'" Id="'+Id+'_'+parseInt(i+1)+'"><img border="0" alt="Delete Attachment" src="/sites/Mrs_Bectors_PTMS/BectorsSourceCode/Images/Delete_Icon.png" width="20" height="20" style="margin-left: 10px;cursor: pointer;"></a>');
		//$('#'+AttachID).append(parseInt(i+1)+'. <a href="'+data[i].Fileurl+'" target="_blank" style="text-decoration:underline;font-weight: bold;">'+data[i].Filename+'</a><a onclick="DeleteObservationFormAttachment(this)" data-ItemId="'+ItemId+'" data-filename="'+data[i].Filename+'" Id="'+Id+'_'+parseInt(i+1)+'"><img border="0" alt="Delete Attachment" src="/sites/Mrs_Bectors_PTMS/BectorsSourceCode/Images/Delete_Icon.png" width="20" height="20" style="margin-left: 10px;cursor: pointer;"></a>');
		$('#'+AttachID).append(parseInt(i+1)+'. <a href="'+data[i].Fileurl+'" target="_blank" style="text-decoration:underline;font-weight: bold;">'+data[i].Filename+'</a><a onclick="DeleteObservationFormAttachment(this)" data-ItemId="'+ItemId+'" data-filename="'+data[i].Filename+'" Id="'+Id+'_'+parseInt(i+1)+'"><img border="0" alt="Delete Attachment" src="/sites/Mrs_Bectors_PTMS/BectorsSourceCode/Images/Delete_Icon.png" width="20" height="20" style="margin-left: 10px;cursor: pointer;"></a><br/>');
		}
	}
	else
	{
