...
function getDepartmentsSuccess(collDepartment)
{
	if(collDepartment.length>0)
	{
		...
		getAreas(selectedDepartmentFilter,getAreasSuccess,getAreasFailure)
	}
...
function getAreasSuccess(collArea)
{
	if(collArea.length>0)
	{
		RawAreaArray = collArea
		var selectedAreaFilter = "&$filter="
		for(var i=0;i<collArea.length;i++)
		{
			if(i>0 && i<collArea.length)
			{
				selectedAreaFilter +=" or "
			}
			selectedAreaFilter += "AreaId eq "+collArea[i].Id
		}
		getCategorys(selectedAreaFilter,getCategorysSuccess,getCategorysFailure)
	}
}
function getAreasFailure()
{
...
function getSeveritySuccess(collSeverity)
{
	...
	var CriteriaTitle = "";
	if(RawAreaArray.length>0)
	{
		var TempDepartmentHtml = '';	
		for(i=0;i<RawAreaArray.length;i++)
		{
			...
			{
				if(RawCategoryArray[j].AreaId == RawAreaArray[i].Id)
				{
					Srno++
					...
					{
						if(RawCriteriaArray[k].AreaId == RawAreaArray[i].Id && RawCriteriaArray[k].CategoryId == RawCategoryArray[j].Id)
						{
							var	newCriteria = tempCriteriaHtml;
							CriteriaTitle = RawCriteriaArray[k].Criteria;
			...
			newDept=newDept.replace(/__DepartmentName__/g,RawAreaArray[i].Title)
			newDept=newDept.replace(/__PlantDepartmentID__/g,RawAreaArray[i].Id);
...
function SubmitCriteriaNonConfirmity(data)
{
	...
	if(DepartmentDetails[0].TourId != 0){
		console.log(DepartmentDetails[0].TourId )
	}else{
		console.log(DepartmentDetails[0].TourId )
	}
...
//alert('okey')
	...
	formID = formID.replace("CheckBoxId","CNCFormID");
	if($(data).is(":checked"))
	{
		$("#"+formID).hide();
	}
...
var tempDepartmentHTML = '<div class="accordion" id="accordionExample___PlantDepartmentID__">'+
    '<div class="card">'+
        '<div class="card-header accordion-header" id="heading___PlantDepartmentID__">'+
            ...
            '</h2>'+
        '</div>'+
        '<div id="collapse___PlantDepartmentID__" class="collapse" aria-labelledby="heading___PlantDepartmentID__" data-parent="#accordionExample___PlantDepartmentID__">'+   
              '<div class="card-body">'+
...
'<div class="row mgn-nill">'+
	...
	'<div class="btn-section">'+
		...
		'<button type="button" class="btn" id="Dept___PlantDepartmentID___Cat___CategoryID___Crit___CriteriaID___BtnId" onclick="SubmitCriteriaNonConfirmity(this)">Save</button>'+
	'</div>'+
'</div>'+   
'</li>'
...
var tempDepartmentHTML = '<div class="accordion" id="accordionExample___PlantDepartmentID__">'+
    '<div class="card">'+
        '<div class="card-header accordion-header" id="heading___PlantDepartmentID__">'+
            ...
            '</h2>'+
        '</div>'+
        '<div id="collapse___PlantDepartmentID__" class="collapse in" aria-labelledby="heading___PlantDepartmentID__" data-parent="#accordionExample___PlantDepartmentID__" style="height: 0px;" aria-expanded="true">'+   
              '<div class="card-body">'+
                '<div class="accordion" id="accordionExample___PlantDepartmentID__">__CriticalCategory__</div>'+
            '</div>'+
        '</div>'+
