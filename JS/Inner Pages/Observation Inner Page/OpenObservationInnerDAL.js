...
function GetObservationReportData(SelectedSevirityId,ObservationId,SelectedDepartmentId,Success,Failure)
{
	var SelectQuery = "?$select=Id,Criteria,Title,Where,DepartmentId/Id,DepartmentId/Title,CategoryId/Id,CategoryId/Title,What,Observation,CorrectiveAction,SeverityId/Id,SeverityId/Title,ObservedBy/Title,ObservedDate,Status,AreaId/Id,AreaId/Title,PlantId/Title";
	//var orderByQuery= "";
	var filterQuery='';

if(SelectedDepartmentId==0 & SelectedSevirityId==0)
	{
  		   if(ObservationId=="")
		   {
			  filterQuery= "&$filter=Status eq 'Pending' and Action eq 'Rejected'";
			}
			else
			{
			  filterQuery= "&$filter=Status eq 'Pending' and Action eq 'Rejected' and CategoryId eq "+ObservationId;
			}
				
	}
	
	else if(SelectedDepartmentId!=0 & SelectedSevirityId!=0)
	{
			if(ObservationId=="")
			{
				filterQuery= "&$filter=Status eq 'Pending' and Action eq 'Rejected' and DepartmentId eq "+SelectedDepartmentId+" and SeverityId eq "+SelectedSevirityId;	
			}
			else
			{
				filterQuery= "&$filter=Status eq 'Pending' and Action eq 'Rejected' and DepartmentId eq "+SelectedDepartmentId+" and CategoryId eq "+ObservationId+" and SeverityId eq "+SelectedSevirityId;		
	...
	else if(SelectedDepartmentId!=0)
	{
		if(ObservationId=="")
		{
			filterQuery= "&$filter=Status eq 'Pending' and Action eq 'Rejected' and DepartmentId eq "+SelectedDepartmentId;	
		}
		else
		{
			filterQuery= "&$filter=Status eq 'Pending' and Action eq 'Rejected' and DepartmentId eq "+SelectedDepartmentId+" and CategoryId eq "+ObservationId;
		}
	}
	...
	else
	{
		if(ObservationId=="")
		{
			filterQuery= "&$filter=Status eq 'Pending' and Action eq 'Rejected' and SeverityId eq "+SelectedSevirityId;	
		}
		else
		{
			filterQuery= "&$filter=Status eq 'Pending' and Action eq 'Rejected' and SeverityId eq "+SelectedSevirityId+" and CategoryId eq "+ObservationId;
		}
	}
		//var ExpandQuery="&$expand=DepartmentId,CategoryId,SeverityId,AreaId,ObservedBy";
...
function getSeverity(Success,Failure)
{
	SelectQuery = "?$select=Id,Title,PlantId/Id,Sequence";
   	var filterQuery= "&$filter= ((Sequence eq 3) or  (Sequence eq 2))";
   		//var filterQuery= "&$filter= Sequence eq '3','4'";
