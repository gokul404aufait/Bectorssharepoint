...
async function startSessionHandler(event) {
  ...
  if (btn && btn.textContent === "Start Session") {
    ...
    const project = startForm.querySelector(`#projectSelect-${cycleNum}`).value;
    const projectOwner = document.getElementById(`project-owner-${cycleNum}`).value;
    ...
    infoWrapper.querySelector('.tour-cyle-start-info').innerHTML = `
            <div class="start-info-item">
                <p class="item-label">Project</p>
                <p class="item-value">${project}</p>
            </div>
            <div class="start-info-item">
                <p class="item-label">Project Owner</p>
                <p class="item-value">${projectOwner}</p>
            </div>
        `;
...
async function fetchCycleData() {
  try {
    ...
    const tableName = "cr953_prod_milestones";
    const apiUrl = `${environmentUrl}/api/data/v${apiVersion}/${tableName}?$filter=cr953_qualitytourid eq '${QualityTourId}'&$select=cr953_projectname,cr953_milestonename,cr953_milestoneduedate,cr953_milestonestatus,cr953_milestone,cr953_projectowner`;
    ...
    data.value.forEach(record => {
      ...
      if (!cycles[cycleNum]) {
        cycles[cycleNum] = {
          cycleNum,
          project: record.cr953_projectname,
          projectOwner: record.cr953_projectowner,
          milestoneName: record.cr953_milestonename,
          milestoneDueDate: record.cr953_milestoneduedate,
          milestoneStatus: record.cr953_milestonestatus,
        };
      }
    });
...
function createCycleSection(cycleNum, isCompleted = false, cycleData = null) {
                ...
                <form class="tour-cyle-info-form">
                    <div class="form-group">
                        ...
                        <div class="select2-parent">
                            <select id="projectSelect-${cycleNum}" class="form-select"
                                name="product-dropdown">
                                <option value="Mrs. Bector's Food Specialists Private Limited">Mrs. Bector's Food Specialists Private Limited</option>
                                <option value="Etihad Aviation Group">Etihad Aviation Group</option>
                                <option value="Triveni Turbines Limited">Triveni Turbines Limited</option>
                                <option value="Pierian Services LLP">Pierian Services LLP</option>
                                <option value="Arjas Steels Private Limited">Arjas Steels Private Limited</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                    <label class="form-label" for="project-owner-${cycleNum}">Project Owner</label>
                    <input type="text" class="form-control" id="project-owner-${cycleNum}" placeholder="" />
                    </div>
                    ...
                    <div
                                                    ...
                                                    <div class="estimation-grid" style="display: flex; gap: 20px;">
                                                        ...
                                                        <div class="form-group" style="flex: 0.3;">
                                                            <label class="form-label" for="milestone-name-${cycleNum}">Milestone Name</label>
                                                            <input type="text" class="form-control" id="milestone-name-${cycleNum}"
                                                        ...
                                                        <div class="form-group" style="flex: 0.7;">
                                                            <label class="form-label" for="milestone-date-${cycleNum}">Milestone Due Date</label>
                                                            <input type="text" class="form-control"
                                                                id="milestone-date-${cycleNum}" placeholder="Enter value"
													...
													<label class="form-label" for="milestone-status-${cycleNum}">
														Milestone Status
													</label>
													<div class="select2-parent">
														<select id="milestone-status-${cycleNum}" class="form-select" name="milestone-status-dropdown">
    ...
    if (!isCompleted) {
      ...
      infoWrapper.innerHTML = `
                <div class="tour-cyle-start-info">
                    <div class="start-info-item">
                        <p class="item-label">Project</p>
                        <p class="item-value">${cycleData.project}</p>
                    </div>
                </div>
            `;
      infoWrapper.style.display = "block";
  ...
  $(`#projectSelect-${cycleNum}`).select2({
            ...
            <div class="bs-card-body">
                <div class="bs-table-container">
                    <table class="bs-table">
                        <thead>
                            <tr>
                                <th colspan="2" style="text-align: center; border: 1px solid #e0e0e0;">Project</th>
                                <th colspan="2" style="text-align: center; border: 1px solid #e0e0e0;">Milestone Name</th>
                                <th colspan="2" style="text-align: center; border: 1px solid #e0e0e0;">Milestone Due Date</th>
                                <th colspan="2" style="text-align: center; border: 1px solid #e0e0e0;">Milestone Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td colspan="2" style="text-align: center; border: 1px solid #e0e0e0;">${cycleData.project}</td>
                                <td colspan="2" style="text-align: center; border: 1px solid #e0e0e0;">${cycleData.milestoneName}</td>
                                <td colspan="2" style="text-align: center; border: 1px solid #e0e0e0;">${cycleData.milestoneDueDate}</td>
                                <td colspan="2" style="text-align: center; border: 1px solid #e0e0e0;">${cycleData.milestoneStatus}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
async function collectEstimationDataCycleSave(cycleNum) {
  ...
  const startDataStr = localStorage.getItem(`cycle-${cycleNum}-start-data`);
    ...
    const project = startData.project || "N/A";
    const projectOwner = startData.projectOwner || "N/A";

  if (card) {
    const milestonenameEl = document.getElementById(`milestone-name-${cycleNum}`);
    const milestonestatusEl = document.getElementById(`milestone-status-${cycleNum}`);
    const milestoneduedateEl = document.getElementById(`milestone-date-${cycleNum}`);
    let data = {
      "cr3ea_qualitytourid": "06888099-db7c-f011-b4cc-000d3af269fb",
      "cr3ea_title": 'Milestone_' + moment().format('MM-DD-YYYY'),
      "cr953_milestone": `Milestone-${cycleNum}`,
      "cr3ea_observedby": UserName || null,
      "cr953_milestonename": milestonenameEl?.value || null,
      "cr953_milestonestatus": milestonestatusEl?.value || null,
      "cr953_milestoneduedate": milestoneduedateEl?.value || null,
      "cr953_projectname": project,
      "cr953_projectowner": projectOwner
    }
    savedData.push(data);
  }
